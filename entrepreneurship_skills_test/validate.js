...
check((11 - 1) === 10 && 10 === 10, 'Entrepreneurial-strength responses score toward 10');
check((11 - 10) === 1 && 1 === 1, 'Entrepreneurial-limiting responses score toward 1');
...
check(!/success_likelihood_test|Success Likelihood Test|success-readiness|کامیابی کے امکان/.test(enPage + urPage), 'No source-template topic residue remains');
...
check(/does not predict or guarantee venture success/.test(enPage) && /پیش گوئی یا ضمانت/.test(urPage), 'Both pages avoid guaranteed-outcome claims');
...
check(home.includes('entrepreneurship_skills_test/index.html'), 'Homepage links to English test');
check(home.includes('entrepreneurship_skills_test/urdu.html'), 'Homepage links to Urdu test');
check(home.includes('Entrepreneurship Skills &amp; Ability Test') && home.includes('کاروباری مہارتوں اور صلاحیت کا ٹیسٹ'), 'Homepage includes bilingual section title');
