...
check((11 - 1) === 10 && 10 === 10, 'Healthy-ambition responses score toward 10');
check((11 - 10) === 1 && 1 === 1, 'Ambition-limiting responses score toward 1');
...
check(!/entrepreneurship_skills_test|Entrepreneurship Skills|entrepreneurship-readiness|کاروباری مہارت/.test(enPage + urPage), 'No source-template topic residue remains');
...
check(/does not measure personal worth/.test(enPage) && /ذاتی قدر کی پیمائش/.test(urPage), 'Both pages avoid personal-worth claims');
check(/sustainable wellbeing/.test(enPage) && /پائیدار صحت/.test(urPage), 'Both pages include sustainable ambition framing');
...
check(home.includes('ambition_skills_test/index.html'), 'Homepage links to English test');
check(home.includes('ambition_skills_test/urdu.html'), 'Homepage links to Urdu test');
check(home.includes('Ambition Skills &amp; Ability Test') && home.includes('بلند حوصلگی کی مہارتوں اور صلاحیت کا ٹیسٹ'), 'Homepage includes bilingual section title');
