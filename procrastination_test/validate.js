...
for(const d of [en,ur]){ok(d.meta.levels[0].min===9&&d.meta.levels.at(-1).min===1,'Result-band order');for(const v of [1,3,4.5,6,7.5,9,10])ok(d.meta.levels.some(l=>v>=l.min&&v<l.max),`Result coverage ${v}`)}
...
for(const name of ['index.html','urdu.html']){const file=path.join(__dirname,name),html=fs.readFileSync(file,'utf8');
  ...
  ok(!/Ethics|Moral|Goal-Setting|Time Management|ethics_test|goal_setting_test|time_management_test/i.test(html),`${name} retains source-test content`);
  ok(html.includes('/assets/favicons/favicon.ico'),`${name} missing favicon metadata`);
  const scripts=[...html.matchAll(/<script(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/script>/g)].map(m=>m[1]);scripts.forEach((code,i)=>new vm.Script(code,{filename:`${name}#${i}`}));
}
const home=fs.readFileSync(path.join(root,'index.html'),'utf8');ok(home.includes('procrastination_test/index.html')&&home.includes('procrastination_test/urdu.html'),'Homepage links');
console.log(JSON.stringify({english:500,urdu:500,subtopics:20,perSubtopic:25,reversePerSubtopic:12,lengths:[20,60,100,500],maximumProcrastinationScore:high,minimumProcrastinationScore:low,bilingualAlignment:'passed',urduScript:'passed',resultBands:'passed',ethicsStructureContract:'passed',inlineScripts:'passed',favicon:'passed',homepageLinks:'passed'},null,2));
