...
for(const c of en.meta.categories){const items=en.questions.filter(q=>q.category===c.id);ok(items.length===25,`${c.id} count`);const rev=items.filter(q=>q.reverse).length;ok(rev>=8&&rev<=10,`${c.id} reverse balance (${rev})`)}
ok(!ur.questions.some(q=>/[A-Za-z]{3}/.test(q.text)),'Urdu-bank script check');
...
for(const name of ['index.html','urdu.html']){const file=path.join(__dirname,name),html=fs.readFileSync(file,'utf8');
  ...
  ok(html.includes('GTM-PD85WRBD'),`${name} missing GTM`);
  const scripts=[...html.matchAll(/<script(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/script>/g)].map(m=>m[1]);
  scripts.forEach((code,i)=>new vm.Script(code,{filename:`${name}#${i}`}));
}
const home=fs.readFileSync(path.join(root,'index.html'),'utf8');ok(home.includes('wisdom_test/index.html')&&home.includes('wisdom_test/urdu.html'),'Homepage links');
console.log(JSON.stringify({english:500,urdu:500,subtopics:20,perSubtopic:25,lengths:[20,60,100,500],highScore:high,lowScore:low,bilingualAlignment:'passed',urduScript:'passed',inlineScripts:'passed',homepageLinks:'passed'},null,2));
