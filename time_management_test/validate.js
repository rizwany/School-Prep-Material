...
for(const name of ['index.html','urdu.html']){const file=path.join(__dirname,name),html=fs.readFileSync(file,'utf8');
  ...
  ok(!/Ethics|Moral|Goal-Setting|goal.setting|ethics_test|goal_setting_test/i.test(html),`${name} retains source-test content`);
...
const home=fs.readFileSync(path.join(root,'index.html'),'utf8');ok(home.includes('time_management_test/index.html')&&home.includes('time_management_test/urdu.html'),'Homepage links');
console.log(JSON.stringify({english:500,urdu:500,subtopics:20,perSubtopic:25,reversePerSubtopic:12,lengths:[20,60,100,500],highScore:high,lowScore:low,bilingualAlignment:'passed',urduScript:'passed',ethicsStructureContract:'passed',inlineScripts:'passed',favicon:'passed',homepageLinks:'passed'},null,2));
